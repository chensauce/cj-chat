<template>
  <div class="body">
    <div class="navbar">
      ...
      <router-link class="nav-item right" tag="div" :to="{path:'/login'}">登录</router-link>
    </div>
    ...
    <div class="input-view">
      <input
        class="input"
        v-model="nickName"
        @blur="handleCheckout(checkout.nickName)"
        type="text"
        placeholder="昵称"
      />
      <div class="icon">
        <van-icon v-if="nickName.length>0" @click="nickName=''" name="close" color="#bbb" />
      </div>
      <span :class="checkout.nickName.showTip?'show':'hidden'" color>{{checkout.nickName.tip}}</span>
    </div>
    <div class="input-view">
      <input
        class="input"
