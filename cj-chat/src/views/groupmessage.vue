<template>
  <div style="background: rgb(238, 238, 238);min-height:100vh">
    <van-nav-bar @click-left="$router.go(-1)">
      ...
      <template #title>
        <span>{{$route.params.group_name}}</span>
      </template>
      <template #right>
        <p style="color:rgba(255,93,91,1)" @click="deleteGroup">退出群聊</p>
      </template>
    </van-nav-bar>
    <div class="body">
      <div class="list">
        <div
          ...
          :key="index"
        >
          <img v-if="!item.isMyMessage" :src="item.dispatcher_avatar" alt="头像" />
          <div
            class="message-view"
...
import { getGroupMessageList } from "@/api/groupMessage";
...
export default {
  ...
  methods: {
    ...
    deleteGroup() {
      deleteGroup(this.$route.params.group_id).then(res => {
        console.log(res);
    ...
    sendMessage() {
      ...
      this.$socket.emit("sendGroupMessage", {
        ...
        dispatcher_avatar: this.$store.getters.userInfo.avatar,
  ...
  async mounted() {
    ...
    getGroupMessageList(this.$route.params.group_id).then(res => {
      console.log(res);
      res.data.groupMessageList.forEach(item => {
        item.dispatcher_id == this.$store.getters.userInfo.user_id
          ? (item.isMyMessage = true)
          : (item.isMyMessage = false);
          item.dispatcher_avatar=item.avatar
      });
      this.messageList = res.data.groupMessageList;
      this.$nextTick(() => {
        window.scroll({
          top: document.documentElement.scrollHeight,
          behavior: "smooth"
    ...
    //接受群聊发来的消息
    this.sockets.subscribe("receiveGroupMessage", data => {
  ...
  destroyed() {
    this.sockets.unsubscribe("receiveGroupMessage");
